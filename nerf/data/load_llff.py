...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True):
    ...
    # modify intrinsics of camera
    sh = imageio.imread(imgfiles[0]).shape
    # poses[:, 4, :] *= 1. / factor
    poses[:2, 4, :] = np.array(sh[:2]).reshape([2, 1])
    poses[2, 4, :] *= 1./factor  # focal / factor
...
def viewmatrix(z, up, pos):
    ...
    vec0 = normalize(np.cross(up, vec2))  # x axis
...
def poses_avg(poses):
    hwf = poses[0, :3, -1:]
    center = poses[:, :3, 3].mean(0)  # camera coordinates center
    z = poses[:, :3, 2].sum(0)
    y = poses[:, :3, 1].sum(0)
    c2w = np.concatenate([viewmatrix(z, y, center), hwf], 1)
    return c2w
...
def render_path_spiral(c2w, up, rads, focal, zrate, rots, N):
    ...
    for theta in np.linspace(0., 2. * np.pi * rots, N+1)[:-1]:
        ...
        z = normalize(c - np.dot(c2w_matrix, np.array([0, 0, -focal, 1.])))
...
def recenter_poses(poses):
    # process of w2c @ intrinsics
    poses_ = poses.copy()
    bottom = np.array([[0, 0, 0, 1.]])  # bottom part of camera to world matrix
    c2w = poses_avg(poses)
    c2w = np.concatenate([c2w[:3, :4], bottom], -2)  # c2w shape to (4, 4)
...
def load_llff_data(
    ...
    poses, bds, images = _load_data(basedir, factor=factor)
    logger.info(f"Loaded {basedir}, {bds.min()}, {bds.max()}")

    # Correct rotation matrix ordering and move variable dim to axis 0
    poses = np.concatenate([poses[:, 1:2, :], -poses[:, 0:1, :], poses[:, 2:, :]], 1)
    poses = np.moveaxis(poses, -1, 0).astype(np.float32)
    images = np.moveaxis(images, -1, 0).astype(np.float32)  # (H, W, C, N)(N, H, W, C)
    ...
    if spherify:
        ...
        if path_zflat:
            zloc = -close_depth * .1
            c2w_path[:3, 3] += zloc * c2w_path[:3, 2]
            rads[2] = 0.
            N_rots = 1
            N_views /= 2
        ...
        render_poses = render_path_spiral(
            c2w_path, up, rads, focal, zrate=.5, rots=N_rots, N=N_views
        )
